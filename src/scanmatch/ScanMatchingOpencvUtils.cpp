...
static int
sm_opencv_abort_error_handler(int status, char const* func_name,
        char const* err_msg, char const* file_name, int line, void*)
{
    fprintf(stderr, "ERROR: in %s (%s:%d)  Message: %s \n", func_name,
            file_name, line, err_msg);
    assert(false);
}
void
sm_setupOpencvErrorHandler()
{
    cvRedirectError(sm_opencv_abort_error_handler);
}
...
{
    CvSize sz = cvGetSize(drawFrame);
    if (sigmaX * scaleFactorXY > sz.width || sigmaY * scaleFactorXY > sz.height)
        return;
        ...
        unsigned char ** line_kernel)
{
    ...
    while (val > 32) {
        kernel_size++;
        val = (int) cvRound(255.0 * exp(-1.0 / (sigma * sigma) * (kernel_size
                * kernel_size)));
    }
    kernel_size = 2 * kernel_size + 1; //we only accounted for half +1 to make size odd


    *square_kernel = (unsigned char *) calloc(kernel_size * kernel_size,
    ...
    *line_kernel = (unsigned char *) calloc(kernel_size, sizeof(unsigned char));
    ...
    for (unsigned int i = 0; i < kernel_size; i++) {
        for (unsigned int j = 0; j < kernel_size; j++) {
            ...
            int val = (int) cvRound(255.0 * exp(-1.0 / (sigma * sigma) * (x * x
                    + y * y)));
...
{
    CvMat cv_kern = cvMat(kernel_size, kernel_size, CV_8UC1, square_kern);
    ...
    for (int i = 0; i < count; i++) {
        {
            ...
            if (window_rect.x < 0 || window_rect.y < 0 || window_rect.x
                    + window_rect.width > imSize.width || window_rect.y
        ...
        bool addKernel)
{
    ...
    if (window_rect.x < 0 || window_rect.y < 0 || window_rect.x
        ...
        cvGetSubRect(im, &window, window_rect);
        if (addKernel)
            cvAdd(&cv_square_kern, &window, &window);
        else
    ...
    if (abs(d.x) > abs(d.y)) {
        ...
        cv_diag_line_kernel = cvMat(stretchedKernSize, 1, CV_8UC1,
                diag_line_kernel);
    } else {
        hor = true;
        ...
        pointOffsetY = 0;
        cv_diag_line_kernel = cvMat(1, stretchedKernSize, CV_8UC1,
                diag_line_kernel);
    }
    ...
    for (int i = 0; i < stretchedKernSize / 2; i++) {
        kern_ind = (int) (i * (double) kernel_size
                / ((double) stretchedKernSize));
    ...
    for (int i = 0; i < count - 1; i++) {
        {
            ...
            if (addKernel)
                cvAdd(&cv_diag_line_kernel, &window, &window);
            else
...
//static vars for the opencvDisplayWrapper

std::deque<sm_opencvDisplayWrapper::sm_opencvDisplayFrame *>
        sm_opencvDisplayWrapper::m_displayDeque;
pthread_mutex_t sm_opencvDisplayWrapper::m_mutex;
pthread_cond_t sm_opencvDisplayWrapper::m_dequeCond;
pthread_mutex_t sm_opencvDisplayWrapper::m_dequeLock;
pthread_t sm_opencvDisplayWrapper::m_displayThread;
bool sm_opencvDisplayWrapper::m_threadRunning = false;

bool
sm_opencvDisplayWrapper::start()
{
    if (m_threadRunning)
        return false;
...
void
sm_opencvDisplayWrapper::unlock()
{
    //cerr <<"UNLOCK" << endl;
...
//copied from libbot
void
_timespec_now(struct timespec *ts)
{
    struct timeval tv;
...
//copied from libbot
void
_timespec_addms(struct timespec *ts, long ms)
{
    int sec = ms / 1000;
    ms = ms - sec * 1000;
...
void
sm_opencvDisplayWrapper::waitForFrame()
{
    //only wait for a short period of time...

    struct timespec next_timeout;
    _timespec_now(&next_timeout);
    _timespec_addms(&next_timeout, 250);
    //  printf("sleep\n");
...
void
sm_opencvDisplayWrapper::announceFrame()
{
    pthread_cond_broadcast(&m_dequeCond);
}

void
sm_opencvDisplayWrapper::display(const char * name, CvArr * frame)
{
    if (!m_threadRunning)
        start();
    lock();
    m_displayDeque.push_front(new sm_opencvDisplayFrame(name, frame));
    unlock();
    announceFrame();
...
void *
sm_opencvDisplayWrapper::m_displayThread_function(void*)
{
    while (true) {
        lock();
